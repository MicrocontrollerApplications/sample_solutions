...
int main(void) {
    ...
    unsigned char button_pressed_counter = 0;
    while(1){
        ...
        if(button == 0){ // is TM pressed?
            ...
            if (button_pressed_counter > 15){
                button_pressed_counter = 0; // reset counter
            }
            
            /* 
             * ~: bitwise NOT => inverts every bit of the variable
             * ~ 0b1010 0011 = 0b0101 1100
            */
            unsigned char led_mask = ~(button_pressed_counter << 2);
            // toggle 
            LATB = led_mask;
        }
    }
